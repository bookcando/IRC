...
ChannelMap List::channelList; // 채널 정보를 저장하는 정적 맵을 선언합니다.
ClientMap List::clientList; // 클라이언트 정보를 저장하는 정적 맵을 선언합니다.

List::List() {} // List 클래스의 생성자입니다. 특별한 초기화 작업이 없습니다.
List::~List() {} // List 클래스의 소멸자입니다. 특별한 정리 작업이 없습니다.
...
bool List::deleteClientList(int fd) {
...
bool List::deleteChannelList(std::string name) {
